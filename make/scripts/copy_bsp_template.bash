...
if [ -d "$CY_BSP_TEMPLATES_DIR" ]; then
    ...
    do
        if ! cmp -s $bsp_file $CY_BSP_DESTINATION_ABSOLUTE/$bsp_file; then
            if [[ -f $CY_BSP_DESTINATION_ABSOLUTE/$bsp_file && $CY_INTERNAL_BSP_TARGET_CREATE_BACK_UP == true ]]; then
                echo "Creating backup file $CY_BSP_DESTINATION_ABSOLUTE/$bsp_file.bak"
                cp -p $CY_BSP_DESTINATION_ABSOLUTE/$bsp_file $CY_BSP_DESTINATION_ABSOLUTE/$bsp_file.bak
            fi
            cp -p $bsp_file $CY_BSP_DESTINATION_ABSOLUTE/$bsp_file
        fi
    done
